...
const getMyWork = async (req, res) => {
  ...
  let myWorks = await PreviousWork.find({
    freelancer: freelancer._id,
  }).populate("freelancer");
...
const udpateMyWork = async (req, res) => {
  ...
  // Update Work
  const updatedWork = await PreviousWork.findByIdAndUpdate(workId, req.body, {
    new: true,
  });
...
const getFreelancer = async (req, res) => {
    ...
    try {
        ...
        // previous work fetch
        const previousWorks = await PreviousWork.find({
            freelancer: freelancer._id
        })
